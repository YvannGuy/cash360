...
export class CapsulesService {
  ...
  async getUserCapsules(): Promise<UserCapsuleRecord[]> {
    try {
      ...
      console.warn('getUserCapsules: fallback localStorage', error)
      try {
  ...
  async addUserCapsules(capsuleIds: string[]): Promise<boolean> {
    try {
      ...
      console.warn('addUserCapsules: fallback localStorage', error)
      try {
        const raw = localStorage.getItem('cash360-user-capsules')
        ...
        localStorage.setItem('cash360-user-capsules', JSON.stringify(merged))
        return true
      } catch {
        return false
      }
...
CREATE POLICY "Users manage their capsules" ON user_capsules
