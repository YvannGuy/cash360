...
export default function UploadDropzone({ onFilesChange, files, error, required = false }: UploadDropzoneProps) {
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (files.length + droppedFiles.length > 3) {
      errors.push(t.uploadDropzone?.maxFiles || 'Vous ne pouvez téléverser que 3 relevés maximum')
    }
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="space-y-2">
          ...
          <div className="text-sm text-gray-600">
            ...
            >
              {t.uploadDropzone?.clickToSelect || 'Cliquez pour sélectionner'}
            </label>{' '}
            {t.uploadDropzone?.orDragAndDrop || 'ou glissez-déposez vos fichiers'}
          </div>
      ...
      {files.length > 0 && (
        <div className="space-y-2">
          <h4 className="text-sm font-medium text-gray-700">{t.uploadDropzone?.selectedFiles || 'Fichiers sélectionnés :'}</h4>
          <ul className="space-y-1">
            {files.map((file, index) => (
              <li key={index} className="flex items-center justify-between bg-gray-50 px-3 py-2 rounded">
                ...
                >
                  {t.uploadDropzone?.remove || 'Supprimer'}
                </button>
              </li>
            ))}
          </ul>
      ...
      <div className="flex items-center justify-center space-x-2 text-xs text-gray-500">
        ...
        <span>{t.uploadDropzone?.secureAndConfidential || 'Sécurisé & confidentiel'}</span>
      </div>
    </div>
  )
}
