...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validation : exactement 3 fichiers PDF
    if (relevesFiles.length !== 3) {
      return NextResponse.json(
        { error: 'Vous devez télécharger exactement 3 fichiers PDF' },
        { status: 400 }
      )
    }
    ...
    for (let i = 0; i < relevesFiles.length; i++) {
      ...
      if (file.type !== 'application/pdf') {
        return NextResponse.json(
          { error: `Le fichier ${i + 1} doit être un PDF` },
          { status: 400 }
        )
      }
      // Vérifier la taille (max 50MB par fichier - limite Supabase)
      if (file.size > 50 * 1024 * 1024) {
        return NextResponse.json(
          { error: `Le fichier ${i + 1} est trop volumineux (max 50MB)` },
          { status: 400 }
        )
      }
    }

    // Vérifier si une analyse existe déjà pour cet utilisateur (créée lors du paiement)
    // On cherche une analyse avec status 'en_cours' et sans fichiers uploadés
    const { data: existingAnalyses, error: checkError } = await supabaseAdmin
      .from('analyses')
    ...
    if (checkError) {
      console.error('Erreur lors de la vérification des analyses existantes:', checkError)
    }

    let analysis: any = null
    let ticket: string = ''

    // Si une analyse existe, vérifier si elle a déjà des fichiers
    if (existingAnalyses && existingAnalyses.length > 0) {
      // Vérifier pour chaque analyse si elle a des fichiers
      for (const existingAnalysis of existingAnalyses) {
        ...
        if (filesError) {
          console.error('Erreur vérification fichiers:', filesError)
          continue
        }

        // Si cette analyse n'a pas de fichiers, on l'utilise
        if (!files || files.length === 0) {
          analysis = existingAnalysis
          ticket = existingAnalysis.ticket.replace('CASH-', '')
          break
        }
      }
    }

    // Si aucune analyse existante sans fichiers n'a été trouvée, créer une nouvelle analyse
    if (!analysis) {
      // Générer un ticket unique
      ticket = generateShortTicket()
      ...
      const { data: newAnalysis, error: analysisError } = await supabaseAdmin
        ...
        .insert({
          ...
          status: 'en_cours',
          progress: 10,
          mode_paiement: 'stripe', // Depuis la boutique, toujours Stripe
          message: null,
          user_id: userId
        })
        .select()
      ...
      if (!newAnalysis) {
        return NextResponse.json(
          { error: 'Erreur lors de la création de l\'analyse dans la base de données' },
          { status: 500 }
        )
      }

      analysis = newAnalysis
      // Extraire le ticket sans le préfixe CASH-
      ticket = analysis.ticket.replace('CASH-', '')
    } else {
      // Utiliser le ticket existant (sans le préfixe CASH-)
      ticket = analysis.ticket.replace('CASH-', '')
    }

    // Créer le dossier dans Supabase Storage (utiliser le ticket sans préfixe)
      ...
      if (uploadError) {
        ...
        for (let j = 0; j < uploadedFiles.length; j++) {
          await supabaseAdmin.storage
            .from('releves')
            .remove([uploadedFiles[j]])
        }
        // Supprimer l'analyse créée
        await supabaseAdmin
          .from('analyses')
          .delete()
          .eq('id', analysis.id)
        return NextResponse.json(
          { error: `Erreur lors de l'upload du relevé ${i + 1}: ${uploadError.message}` },
          { status: 500 }
        )
      }
    ...
    const clientEmailHtml = generateClientEmailHtml(clientInfo, ticket)
    await sendMail({
      to: userEmail,
      subject: `Cash360 – Confirmation de réception de vos documents – CASH-${ticket}`,
    ...
    const adminEmailHtml = generateAdminEmailHtml(clientInfo, ticket, timestamp)
    await sendMail({
      to: process.env.MAIL_ADMIN || process.env.DESTINATION_EMAIL || 'cash@cash360.finance',
      subject: `[Cash360] Nouveaux documents reçus – ${userFirstName} ${userLastName} – CASH-${ticket}`,
      html: adminEmailHtml
    })
    ...
    console.error('Erreur API upload-analysis:', error)
