...
export async function POST(request: NextRequest) {
  try {
    ...
    const { preview, confirm, templateId, subject, body: emailBody, isHtml } = body
    ...
    // Traiter le corps selon le mode (HTML ou texte)
    ...
    const emailHtml = generateEmailHtml(templateIdNum, subject, processedBody, origin)
    ...
    // Récupérer uniquement les utilisateurs avec email validé
    ...
    while (hasMore) {
      ...
      const validUsers = batch.filter(user => 
        user.email && 
    ...
    console.log(`[SEND-EMAIL] 📧 Préparation envoi à ${allUsers.length} utilisateurs avec email validé`)
    ...
    for (let i = 0; i < allUsers.length; i++) {
      ...
      try {
        ...
        if ((i + 1) % 10 === 0) {
          console.log(`[SEND-EMAIL] ✅ ${i + 1}/${allUsers.length} emails envoyés...`)
        }
        ...
        console.error(`[SEND-EMAIL] ❌ Erreur pour ${user.email}:`, error)
      }
    }

    console.log(`[SEND-EMAIL] ✅ Envoi terminé: ${results.success} succès, ${results.failed} échecs`)
